...
// ---------- LOCAL date helpers (fallback only; sheet-driven lock is primary) ----------
...
function getMinBookableWeekStartISO() {
  const lead = addDaysLocal(
    startOfDayLocal(),
    Number(process.env.NEXT_PUBLIC_LEAD_TIME) * 7 + 4, // fallback only
  );
            ...
            <div key={wi} className="grid grid-cols-5 gap-1">
              ...
              {week.map(({ dayNum, date }) => {
                ...
                const mealArr = booking
                  ...
                  : [];
                const mealColors = ["#6366f1", "#10b981", "#f59e0b"];
                        ...
                        {mealArr.map((booked, j) =>
                          booked ? (
                            <div
                              ...
                              style={{ backgroundColor: mealColors[j] }}
                            />
                          ) : null,
                        )}
                      </div>
